...
def invert(input_dict: dict[str, str]) -> dict[str, str]:
    ...
    list_idx1: int = 0
    list_idx2: int = 0
    while repeat is False and list_idx2 < len(value_list):
        list_idx1 = list_idx2 + 1
        while repeat is False and list_idx1 < len(value_list):
...
def count(input_list: list[str]) -> dict[str, int]:
    """Counts the number each time a str occurs in a given list and returns a dictionary of the counts."""
